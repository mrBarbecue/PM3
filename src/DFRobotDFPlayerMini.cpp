...
DFRobotDFPlayerMini::DFRobotDFPlayerMini(PinName RX, PinName TX, PinName BUSY)
    : serial(TX, RX, 9600), BusyPin(BUSY)
{

}

bool DFRobotDFPlayerMini::getBusyState(){
    //Return true if playing
    return !BusyPin.read();
}

bool DFRobotDFPlayerMini::getBaneModeState(){
    return baneModeActive;
}
void DFRobotDFPlayerMini::setBaneModeState(){
    baneModeActive = !baneModeActive;
}
...
void DFRobotDFPlayerMini::sendStack(){
  ...
  printf("\n");
#endif
  serial.write(_sending, DFPLAYER_SEND_LENGTH);
  _timeOutTimer  = Kernel::get_ms_count();
...
bool DFRobotDFPlayerMini::waitAvailable(unsigned long duration){
  unsigned long timer = Kernel::get_ms_count();
  if (!duration) {
    duration = _timeOutDuration;
  }
  while (!available()){
    if (Kernel::get_ms_count() - timer > duration) {
